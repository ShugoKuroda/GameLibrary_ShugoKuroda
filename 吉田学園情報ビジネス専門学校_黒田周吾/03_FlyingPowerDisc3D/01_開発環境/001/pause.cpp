...
//	Player *pPlayer = GetPlayer();
	...
	if (GetKeyboardTrigger(DIK_P) == true || GetJoypadTrigger(JOYKEY_START, PLAYER_1) && g_bPause == false)
	{
		//ゲームを止める
		g_nCntCol = 0;
		g_bPause = true;
	...
	if (g_bPause == true)
	{
		...
		{//ポーズ選択画面
			if (GetKeyboardTrigger(DIK_LEFT) == true || GetJoypadTrigger(JOYKEY_LEFT, PLAYER_1))
			{
				g_nPauseSelecting--;
				//選択していない項目のリセット
				g_fColPauseA[2] = 1.0f;
			...
			else if (GetKeyboardTrigger(DIK_RIGHT) == true || GetJoypadTrigger(JOYKEY_RIGHT, PLAYER_1))
			{
				g_nPauseSelecting++;
				//選択していない項目のリセット
				g_fColPauseA[2] = 1.0f;
			...
			else if (GetKeyboardTrigger(DIK_RETURN) || GetKeyboardTrigger(DIK_SPACE) == true || GetJoypadTrigger(JOYKEY_A, PLAYER_1))
			{
				if (g_nPauseSelecting == 2)
				{//ゲームを続ける
					g_bPause = false;
