...
#define PLAYER_MAX		(PLAYER_MAX)		//プレイヤーの最大人数
...
D3DXVECTOR3 GetJoypadStick(JOYKEY Key, int nPlayer)
{
	...
	case JOYKEY_LEFT_STICK:
		if (nPlayer == PLAYER_1)
		{//プレイヤー１の場合
			g_JoyStickPos[nPlayer] = D3DXVECTOR3(g_JoyKeyState[nPlayer].Gamepad.sThumbLX / 32767.0f, 0.0f, g_JoyKeyState[nPlayer].Gamepad.sThumbLY / 32767.0f);
		}
	...
	case JOYKEY_RIGHT_STICK:
		g_JoyStickPos[nPlayer] = D3DXVECTOR3(g_JoyKeyState[nPlayer].Gamepad.sThumbRX / 32767.0f, 0.0f, g_JoyKeyState[nPlayer].Gamepad.sThumbRY / 32767.0f);
		break;
	}
